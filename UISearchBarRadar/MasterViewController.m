...
{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Test Failed" message:message delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
    [alertView show];
}
...
{
    NSString *errorMessage = [NSString stringWithFormat:@"AutocapitalizationType not expected value. Expecting %d got %d",
                              (int)UITextAutocapitalizationTypeNone, (int)searchBar.autocapitalizationType];
    NSLog(@"%@", errorMessage);
    if(searchBar.autocapitalizationType != UITextAutocapitalizationTypeNone)
    {
        [self showAlertViewWithMessage:errorMessage];
    }
    return YES;
